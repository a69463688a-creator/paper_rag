...
class RAGSystem:
    ...
    def generate_answer(self,query,source_filter=None):
        ...
        if query_category=='通用知识':
            logger.info('查询为通用知识,直接使用LLM生成答案')
            ...
            try:
                ...
                answer=f'抱歉,处理通用知识出错,请联系人工客服:{conf.CUSTOMER_SERVICE_PHONE}'
        ...
        try:
            ...
            logger.error(f'直接调用LLM失败:{e}')
            answer = f'抱歉,处理专业咨询出错,请联系人工客服:{conf.CUSTOMER_SERVICE_PHONE}'
        processing_time = time.time() - start_time
        logger.info(f'专业咨询查询处理完成,耗时:{processing_time:.3f}s,查询问题:{query}')
        return answer
