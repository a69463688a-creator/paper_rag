...
class RAGSystem:
    ...
    def generate_answer(self,query,source_filter=None):
        ...
        if query_category=='通用问答' and any(kw.lower() in query.lower() for kw in paper_keywords):
            ...
            logger.info(f'关键词规则兜底: {query_category} (查询: {query})')

        if query_category=='通用问答':
            logger.info('查询为通用问答,直接使用LLM生成答案')
            ...
            try:
                ...
                answer=f'抱歉,处理通用问答出错,请联系人工客服:{conf.CUSTOMER_SERVICE_PHONE}'
            processing_time=time.time()-start_time
            logger.info(f'通用问答查询处理完成,耗时:{processing_time:.3f}s,查询问题:{query}')
            return answer
        logger.info('查询为论文学术咨询,执行RAG流程')
        ...
        try:
            ...
            logger.error(f'直接调用LLM失败:{e}')
            answer = f'抱歉,处理论文学术咨询出错,请联系人工客服:{conf.CUSTOMER_SERVICE_PHONE}'
        processing_time = time.time() - start_time
        logger.info(f'论文学术咨询查询处理完成,耗时:{processing_time:.3f}s,查询问题:{query}')
        return answer
